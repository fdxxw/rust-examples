...
impl Entities {
    ...
    pub fn create_entity(&mut self) -> &mut Self {
        if let Some((index,_)) = self.map.iter().enumerate().find(|(index, mask)|**mask == 0) {
            self.insertin_into_index = index;
        } else {
            self.components
                .iter_mut()
                .for_each(|(key, components)| components.push(None));
            self.map.push(0);
            self.insertin_into_index = self.map.len() - 1;
        }
        self
    }
    ...
    pub fn with_component(&mut self, data: impl Any) -> Result<&mut Self> {
        ...
        let index = self.insertin_into_index;
    ...
    pub fn has_component(&self, index: usize, mask: u32) -> bool {
...
mod tests {
    ...
    #[test]
    fn bigmask_updated_when_registering_entities() {
        let mut entities = Entities::default();
        entities.register_component::<Health>();
        let type_id = TypeId::of::<Health>();
    ...
    #[test]
    fn created_entities_ae_inserted_into_deleted_entities_columes() -> Result<()> {
        let mut entities = Entities::default();
        entities.register_component::<Health>();
        entities.create_entity().with_component(Health(100))?;
